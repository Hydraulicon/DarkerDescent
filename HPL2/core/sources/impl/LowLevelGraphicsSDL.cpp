...
// SDL3: SDL_syswm.h removed - not needed in this file
...
namespace hpl {
		...
		const cVector2l& avWindowPos)
	{
		...
		Uint64 mlFlags = SDL_WINDOW_OPENGL;
		if (alWidth == 0 && alHeight == 0) {
			mvScreenSize = cVector2l(800, 600);
			mlFlags |= SDL_WINDOW_FULLSCREEN;  // SDL3: FULLSCREEN_DESKTOP removed
		}
		...
		// SDL3: SDL_CreateWindow signature changed - takes (title, width, height, flags)
		mpScreen = SDL_CreateWindow(asWindowCaption.c_str(),
			mvScreenSize.x, mvScreenSize.y, mlFlags);
		// SDL3: Center window after creation if not fullscreen
		if (mpScreen && !abFullscreen) {
		...
		// SDL3: SDL_SetWindowBrightness removed - use SDL_SetWindowOpacity if needed
		// Or handle gamma in shaders
	...
	void cLowLevelGraphicsSDL::SetWindowGrab(bool abX)
	{
		mbGrab = abX;
		if (mpScreen) {
			SDL_SetWindowMouseGrab(mpScreen, abX ? true : false);
		}
	}

	void cLowLevelGraphicsSDL::SetRelativeMouse(bool abX)
	{
		// SDL3: SDL_SetRelativeMouseMode → SDL_SetWindowRelativeMouseMode
		if (mpScreen) {
	...
	{
		// SDL3: SDL_WINDOW_SHOWN removed - check if NOT hidden instead
		return mpScreen && !(SDL_GetWindowFlags(mpScreen) & SDL_WINDOW_HIDDEN);
	}
	...
	void cLowLevelGraphicsSDL::SetGammaCorrection(float afX)
	{
		...
		// SDL3: SDL_SetWindowBrightness removed - handle gamma in shaders if needed
		// SDL_SetWindowBrightness(mpScreen, mfGammaCorrection);
	}
