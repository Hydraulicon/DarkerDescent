...
namespace hpl {
	...
	void cKeyboardSDL::Update()
	{
		...
		for (; it != mpLowLevelInputSDL->mlstEvents.end(); ++it)
		{
			...
			if (pEvent->type == SDL_EVENT_KEY_DOWN || pEvent->type == SDL_EVENT_KEY_UP)
			{
				...
				// SDL3: Check event type instead of key.state
				mvKeyArray[key] = (pEvent->type == SDL_EVENT_KEY_DOWN);

				// SDL3: key.keysym.mod → key.mod (still a Uint16)
	...
	*/
	...
	{
		// SDL3: Most key constants unchanged from SDL2
		switch (alKey)
		{
		...
		case 	SDLK_GRAVE: return eKey_BackQuote;
		...
		case 	SDLK_DELETE: return eKey_Delete;
			// SDL3: Keypad keys unchanged from SDL2
	...
	void cKeyboardSDL::AddKeyToList(int alSDLMod, eKey aKey, int alUnicode, std::list<cKeyPress>& alstKeys)
	{
		...
		// SDL3: Modifier constants now have SDL_ prefix
